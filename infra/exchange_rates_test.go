...
func TestExchangeRateService_GetRate_SameCurrency(t *testing.T) {
	...
	service := NewExchangeRateService([]domain.ExchangeRateProvider{}, cache.NewMemoryCache(), logger)
...
func TestExchangeRateService_GetRate_FromCache(t *testing.T) {
	...
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	service := NewExchangeRateService([]domain.ExchangeRateProvider{}, cache, logger)
