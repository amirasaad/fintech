...
func Validation(bus eventbus.Bus, logger *slog.Logger) func(ctx context.Context, e common.Event) error {
	return func(ctx context.Context, e common.Event) error {
		...
		if tr.AccountID == uuid.Nil || tr.DestAccountID == uuid.Nil || tr.UserID == uuid.Nil || tr.Amount.IsZero() || tr.Amount.IsNegative() {
		...
		validatedEvent := events.NewTransferValidatedEvent(
			...
			tr.AccountID,
			tr.CorrelationID,
			events.WithTransferRequestedEvent(*tr),
		)
		validatedEvent.FlowEvent.UserID = tr.UserID
		validatedEvent.FlowEvent.AccountID = tr.AccountID
