...
func TestPaymentInitiationHandler_Scenarios(t *testing.T) {
	tests := []struct {
		name      string
		input     domain.Event
		cancelCtx bool
		...
		{
			name:      "valid MoneyConvertedEvent",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			bus := &mockEventBus{}
			handler := PaymentInitiationHandler(bus)
			...
			if tc.expectPub {
				...
				_, ok := bus.published[0].(accountdomain.PaymentInitiatedEvent)
