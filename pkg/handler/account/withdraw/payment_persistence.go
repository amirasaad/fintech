...
func PaymentPersistenceHandler(bus eventbus.EventBus, uow repository.UnitOfWork, logger *slog.Logger) func(context.Context, domain.Event) {
	return func(ctx context.Context, e domain.Event) {
		logger := logger.With("handler", "WithdrawPaymentPersistenceHandler")
		...
		default:
			logger.Error("unexpected event type for withdraw payment persistence", "event", e)
			return
		}
		...
		err = uow.Do(ctx, func(uow repository.UnitOfWork) error {
			...
			logger.Info("transaction updated with payment ID", "transaction_id", transactionID)
			return nil
		})
		...
		logger.Info("payment ID persisted successfully", "transaction_id", transactionID)
	}
}
