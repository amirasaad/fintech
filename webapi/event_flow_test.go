...
func (s *E2EFlowsTestSuite) TestDepositE2E() {
	...
	s.Require().True(
		ok,
		"Expected accountResp['data'] to be a map, got: %#v",
		accountResp["data"],
	)
...
func (s *E2EFlowsTestSuite) TestWithdrawE2E() {
	...
	s.Require().True(
		ok,
		"Expected data['ID'] to be a string, got: %#v",
		data["ID"],
	)
	...
	"external_target":
	{"bank_account_number":"123456789","routing_number":"987654321"},
	"money_source": "cash"}`
	resp = s.MakeRequest(
		"POST",
...
func (s *E2EFlowsTestSuite) TestTransferE2E() {
	...
	s.Require().True(
		ok,
		"Expected accountResp1['data'] to be a map, got: %#v",
		accountResp1["data"],
	)
	...
	s.Require().True(
		ok,
		"Expected accountResp2['data'] to be a map, got: %#v",
		accountResp2["data"],
	)
	...
	s.Require().True(
		ok,
		"Expected data2['ID'] to be a string, got: %#v",
		data["ID"],
	)
	...
	resp = s.MakeRequest(
		"POST",
		fmt.Sprintf("/account/%s/deposit", accountID1),
		depositBody,
		token,
	)
	if resp.StatusCode != http.StatusAccepted {
	...
	transferBody := fmt.Sprintf(
		`{"amount":100,"currency":"USD","destination_account_id":"%s", "money_source": "cash"}`,
		accountID2,
	)
	resp = s.MakeRequest(
		"POST",
		fmt.Sprintf("/account/%s/transfer", accountID1),
		transferBody,
		token,
	)
	if resp.StatusCode != http.StatusAccepted {
